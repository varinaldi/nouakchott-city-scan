...
// plot_wsf_harmonized
// ------------------------------------------------------------
function plot_wsf_harmonized(data, {
  cityName = globalCity,
  width = plotWidth,
  height = null,
  yLabel = "Urban Built-up Area (sq km)",
  xLabel = "Year",
  evoSource = "WSF Evolution"
} = {}) {
  ...
  const evoData = data.filter(d => d.source === evoSource);
  const trackerData = data.filter(d => d.source === "WSF Tracker");

  const allData = [...evoData, ...trackerData];
  const minYear = d3.min(allData, d => d.year);
  const maxYear = d3.max(allData, d => d.year);
  const maxVal = d3.max(allData, d => d.cumulative_sq_km);

  // Scales — use actual year
  const xScale = d3.scaleLinear()
  ...
  // Line config
  const lines = [
    { data: evoData, color: "#2166ac", dash: "6,3", label: evoSource },
    { data: trackerData, color: "#b2182b", dash: "6,3", label: "WSF Tracker" }
  ];
  ...
  yAxisGroup.call(d3.axisLeft(yScale)
    ...
    .tickFormat(d3.format("~s")));
  yAxisGroup.select(".domain").remove();
  yAxisGroup.selectAll(".tick line").attr("stroke", "#f0f0f0").attr("stroke-width", 1);
  yAxisGroup.selectAll(".tick text")
    .attr("fill", "currentColor").attr("font-size", "10px")
    .attr("font-family", "system-ui").attr("dx", "-0.3em");
  ...
  xAxisGroup.call(d3.axisBottom(xScale)
    ...
    .tickSizeOuter(0)
    .tickSizeInner(-innerHeight));
  xAxisGroup.select(".domain").attr("stroke", "currentColor").attr("stroke-width", 1);
  xAxisGroup.selectAll(".tick line").attr("stroke", "#f0f0f0").attr("stroke-width", 1);
  xAxisGroup.selectAll(".tick text")
    .attr("fill", "currentColor").attr("font-size", "10px")
    .attr("font-family", "system-ui").attr("dy", "1.5em");

  // Draw lines
  const lineGen = d3.line()
    .defined(d => d.cumulative_sq_km !== null && d.cumulative_sq_km !== undefined)
    .x(d => xScale(d.year))
    .y(d => yScale(d.cumulative_sq_km));

  const strokeWidth = Math.max(1.5, Math.min(3, width / 400));
  const dotRadius = Math.max(2, Math.min(3, width / 200));
  ...
  lines.forEach(l => {
    ...
    g.selectAll(null)
      .data(l.data.filter(d => d.cumulative_sq_km !== null))
      .enter().append("circle")
      .attr("cx", d => xScale(d.year))
      .attr("cy", d => yScale(d.cumulative_sq_km))
      .attr("r", dotRadius)
      .attr("fill", l.color)
      .style("cursor", "default")
      .on("mouseover", function(event, d) {
        ...
        const tooltip = d3.select("body").append("div")
          ...
          .style("z-index", "1000")
          .style("opacity", 0);
        tooltip.html(`<div>${l.label}</div><div>Year: ${d.year}</div><div>${yLabel}: ${d.cumulative_sq_km.toLocaleString()}</div>`)
          .style("left", (event.pageX - 60) + "px")
          .style("top", (event.pageY - 60) + "px")
      ...
      .on("mousemove", function(event) {
        const tooltip = d3.select(".d3-tooltip");
        if (!tooltip.empty()) {
          tooltip.style("left", (event.pageX - 60) + "px").style("top", (event.pageY - 60) + "px");
        }
      })
      .on("mouseout", function() {
  ...
  // Legend (no border, like plot_comb)
  ...
  lines.forEach((l, i) => {
    ...
    legendItem.append("text")
      .attr("x", 25).attr("y", 0).attr("dy", "0.32em")
      .attr("font-size", "12px").attr("font-family", "system-ui").attr("fill", "currentColor")
      .text(l.label);
  });
  ...
  svg.append("text")
    ...
    .attr("font-size", "18px").attr("font-family", "system-ui")
    .text(`Urban Built-up Area, ${minYear}-${maxYear}`);

  svg.append("text")
    .attr("x", 20).attr("y", 45)
    .attr("text-anchor", "start").attr("fill", "currentColor")
    .attr("font-size", "16px").attr("font-family", "system-ui").attr("font-style", "italic")
    .text(cityName);

  // Y-axis label
  svg.append("text")
    .attr("transform", "rotate(-90)")
    .attr("y", 20).attr("x", -(margin.top + innerHeight / 2))
    .attr("text-anchor", "middle").attr("fill", "currentColor")
    .attr("font-size", "11px").attr("font-family", "system-ui")
    .text(yLabel);
...
function plot_aq(aq_area, {
    ...
    g.selectAll(".data-bar")
      ...
      .attr("stroke", d => d3.color(color).darker(0.5))
      .attr("stroke-width", 1)
      .style("cursor", "default")
      .on("mouseover", function(event, d) {
        // remove any existing tooltips
...
function plot_flood_prob(data, {
  ...
  // Draw lines for each probability bin (split at 2015/2016 for WSF source change)
  bins.forEach(bin => {
    ...
    const evoSegment = lineData.filter(d => d.yearName <= 2015);
    const trackerSegment = lineData.filter(d => d.yearName >= 2016);

    [evoSegment, trackerSegment].forEach(segment => {
...
function plot_fu(fu, {
  ...
  // Split at 2015/2016 for WSF source change
  const evoData = fu.filter(d => d.yearName <= 2015);
  const trackerData = fu.filter(d => d.yearName >= 2016);
  [evoData, trackerData].forEach(segment => {
    if (segment.length === 0) return;
    g.append("path")
      .datum(segment)
      .attr("fill", "none")
...
function plot_pu(pu, {
  ...
  // Split at 2015/2016 for WSF source change
  const evoData = pu.filter(d => d.yearName <= 2015);
  const trackerData = pu.filter(d => d.yearName >= 2016);
  [evoData, trackerData].forEach(segment => {
    if (segment.length === 0) return;
    g.append("path")
      .datum(segment)
...
function plot_cu(cu, {
  ...
  // Split at 2015/2016 for WSF source change
  const evoData = cu.filter(d => d.yearName <= 2015);
  const trackerData = cu.filter(d => d.yearName >= 2016);
  [evoData, trackerData].forEach(segment => {
    if (segment.length === 0) return;
    g.append("path")
      .datum(segment)
...
function plot_comb(comb, pu, fu, cu, {
  ...
  // combined line (comb) - black solid (split at 2015/2016)
  const combLine = d3.line()
    .defined(d => d.comb !== null)
    .x(d => xScale(d.year))
    .y(d => yScale(d.comb));
  [comb.filter(d => d.yearName <= 2015), comb.filter(d => d.yearName >= 2016)].forEach(segment => {
    if (segment.length === 0) return;
    g.append("path")
      .datum(segment)
      .attr("fill", "none")
  ...
  // coastal line (cu) - green dashed (split at 2015/2016)
  const cuLine = d3.line()
    .defined(d => d.cu !== null)
    .x(d => xScale(d.year))
    .y(d => yScale(d.cu));
  [cu.filter(d => d.yearName <= 2015), cu.filter(d => d.yearName >= 2016)].forEach(segment => {
    if (segment.length === 0) return;
    g.append("path")
      .datum(segment)
      .attr("fill", "none")
  ...
  // fluvial line (fu) - red dashed (split at 2015/2016)
  const fuLine = d3.line()
    .defined(d => d.fu !== null)
    .x(d => xScale(d.year))
    .y(d => yScale(d.fu));
  [fu.filter(d => d.yearName <= 2015), fu.filter(d => d.yearName >= 2016)].forEach(segment => {
    if (segment.length === 0) return;
    g.append("path")
      .datum(segment)
      .attr("fill", "none")
  ...
  // pluvial line (pu) - blue dashed (split at 2015/2016)
  const puLine = d3.line()
    .defined(d => d.pu !== null)
    .x(d => xScale(d.year))
    .y(d => yScale(d.pu));
  [pu.filter(d => d.yearName <= 2015), pu.filter(d => d.yearName >= 2016)].forEach(segment => {
    if (segment.length === 0) return;
    g.append("path")
      .datum(segment)
      .attr("fill", "none")
...
function plot_ee(ee, {
  ...
  // y-axis with grid lines
  const yTickInterval = 20;  // Show every 20 years
  const yTickValues = d3.range(1900, 2030, yTickInterval);
  const yAxisGroup = g.append("g").attr("class", "y-axis");
  const yAxisCall = d3.axisLeft(yScale)
    .tickValues(yTickValues)
    .tickFormat(d3.format("d"))
